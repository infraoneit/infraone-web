#!/usr/bin/env node
/**
 * Cloud-Telefonie Spoke Wortzahl-Audit
 * Liest alle JSON-Files unter content/leistungen/cloud-telefonie-regions/
...
const regionsDir = path.resolve(__dirname, '../content/leistungen/cloud-telefonie-regions');
...
console.log('Cloud-Telefonie Spoke Wortzahl-Audit');
...
for (const r of results) {
    ...
    console.log(
        `${r.slug.padEnd(13)} | ${String(r.total).padStart(5)} | ${String(r.intro).padStart(5)} | ${String(r.whyChooseUs).padStart(5)} | ${String(r.localIndustries).padStart(10)} | ${String(r.additionalFaqs).padStart(4)} | ${String(r.localFaq).padStart(8)} | ${status}`
    );
}
console.log('');
