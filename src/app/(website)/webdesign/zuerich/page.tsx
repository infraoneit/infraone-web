...
export const metadata: Metadata = {
    title: 'Webdesign Zürich | Websites ab CHF 990',
...
export default function WebdesignZuerichPage() {
    const serviceSchema = generateWebdesignVirtualSpokeSchema('zuerich', 'Zürich', `${BASE_URL}/#business-winterthur`, ['Zürich', 'Grossraum Zürich']);
    const productSchemas = generateWebdesignPackageProducts();
    const offerCatalogSchema = generateOfferCatalogSchema(packages);
    const spokeFaqs = generateWebdesignSpokeFaqs('Zürich', false);
    const faqSchema = generateFAQSchema(spokeFaqs);
    const breadcrumbSchema = generateBreadcrumbListSchema([{ name: 'Home', url: BASE_URL }, { name: 'Webdesign', url: `${BASE_URL}/webdesign` }, { name: 'Zürich', url: `${BASE_URL}/webdesign/zuerich` }]);
    const webPageSchema = generateWebPageSchema(`${BASE_URL}/webdesign/zuerich`, 'Webdesign Zürich', 'Professionelle Websites für Zürcher Unternehmen.');
