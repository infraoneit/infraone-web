...
export default function WebdesignZugPage() {
    const serviceSchema = generateWebdesignVirtualSpokeSchema('zug', 'Zug', `${BASE_URL}/#business-winterthur`, ['Zug', 'Kanton Zug']);
    const productSchemas = generateWebdesignPackageProducts();
    const offerCatalogSchema = generateOfferCatalogSchema(packages);
    const spokeFaqs = generateWebdesignSpokeFaqs('Zug', false);
    const faqSchema = generateFAQSchema(spokeFaqs);
    const breadcrumbSchema = generateBreadcrumbListSchema([{ name: 'Home', url: BASE_URL }, { name: 'Webdesign', url: `${BASE_URL}/webdesign` }, { name: 'Zug', url: `${BASE_URL}/webdesign/zug` }]);
    const webPageSchema = generateWebPageSchema(`${BASE_URL}/webdesign/zug`, 'Webdesign Zug', 'Premium Websites für Zuger Unternehmen.');
