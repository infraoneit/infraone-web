...
export default function WebdesignBernPage() {
    const serviceSchema = generateWebdesignVirtualSpokeSchema('bern', 'Bern', `${BASE_URL}/#business-winterthur`, ['Bern', 'Region Bern']);
    const productSchemas = generateWebdesignPackageProducts();
    const offerCatalogSchema = generateOfferCatalogSchema(packages);
    const spokeFaqs = generateWebdesignSpokeFaqs('Bern', false);
    const faqSchema = generateFAQSchema(spokeFaqs);
    const breadcrumbSchema = generateBreadcrumbListSchema([{ name: 'Home', url: BASE_URL }, { name: 'Webdesign', url: `${BASE_URL}/webdesign` }, { name: 'Bern', url: `${BASE_URL}/webdesign/bern` }]);
    const webPageSchema = generateWebPageSchema(`${BASE_URL}/webdesign/bern`, 'Webdesign Bern', 'Remote-Betreuung mit persönlicher Beratung.');
